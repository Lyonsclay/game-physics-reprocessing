...
let getNewVelocity =
    ...
    : velocityT =>
  switch (velocity) {
  | float when speed(velocity) > terminalSpeed => {x: 0.0, y: 0.0}
...
let draw = ({player} as state, env) => {
  ...
  let posX = player.positiion.x;
  let posY = player.positiion.y;
  ...
  let velocity = player.velocity;
  {
    player: {
      position: {
        ...
        y:
          position.y > 500. ?
            500.0 : position.y +. velocity.y *. Env.deltaTime(env)
      },
      velocity:
        Env.key(Space, env) ?
          getNewVelocity(velocity, {x: 0, y: (-50.0)}, Env.deltaTime(env)) :
          getNewVelocity(velocity, {x: 0, y: 50.0}, Env.deltaTime(env))
    }
  };
};
